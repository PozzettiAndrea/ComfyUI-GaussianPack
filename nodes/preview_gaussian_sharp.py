...
class PreviewGaussianSharp(io.ComfyNode):
    """
    Preview SHARP Gaussian Splatting PLY files (gsplat.js viewer).

    Copy of GeometryPack's Preview Gaussian: takes EXTRINSICS/INTRINSICS and
    reproduces the captured camera view (camera at the extrinsics-derived
    position looking down +Z, FOV from intrinsics). Best for SHARP / monocular
    depth-fan gaussians, which only look correct from the capture origin.
    """
...
NODE_CLASS_MAPPINGS = {
    "PreviewGaussianSharp": PreviewGaussianSharp,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "PreviewGaussianSharp": "Preview Gaussian Sharp",
}
